...
GPU_ID="${1:-7}"
OUT_ROOT="${2:-/data0/multi_rvos_benchmark_v1/runs/videomme_1b_vqa_mem_feedback_v1}"
MODEL_PATH="${3:-/data0/pretrained/Sa2VA-1B}"
DENYLIST_PATH="${4:-$OUT_ROOT/denylist.txt}"
...
run_rating() {
  local xlsx="$OUT_ROOT/feedback/Sa2VA-1B-LOCAL-MEM/Sa2VA-1B-LOCAL-MEM_Video-MME_8frame_nopack_nosubs.xlsx"
  if [[ ! -f "$xlsx" ]]; then
...
if [[ ! -f "$DENYLIST_PATH" ]]; then
  echo "[FAIL_FAST] MISSING_DENYLIST path=$DENYLIST_PATH" | tee -a "$FULL_LOG"
  exit 2
fi

mkdir -p "$OUT_ROOT/feedback"
clear_mode_env
...
echo "[PROGRESS] START mode=feedback data=Video-MME gpu=$GPU_ID mem_enable=${VQA_MEM_ENABLE} model=$MODEL_PATH adapter=${PL_ADAPTER_CKPT:-off} gate=${PL_GATE_MODE:-off} denylist=$DENYLIST_PATH" | tee -a "$FULL_LOG"
...
"$PYTHON_BIN" -u "$ROOT_DIR/tools/run_sa2va_eval_local_mem.py" \
  ...
  --work-dir "$OUT_ROOT/feedback" \
  2>&1 | tee -a "$FULL_LOG"

rc=${PIPESTATUS[0]}
if [[ $rc -ne 0 ]]; then
  echo "[FAIL_FAST] MODE_FAILED mode=feedback rc=$rc" | tee -a "$FULL_LOG"
  exit $rc
fi

run_rating
echo "[PROGRESS] MODE_DONE mode=feedback" | tee -a "$FULL_LOG"
